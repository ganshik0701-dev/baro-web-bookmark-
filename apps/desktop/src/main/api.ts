...
export async function fetchMe(): Promise<{ chromeProfile: string | null } | null> {
  ...
  try {
    ...
    const body = (await res.json()) as { data?: { chromeProfile?: string | null } }
    return { chromeProfile: body.data?.chromeProfile ?? null }
  } catch {
    return null
  }
}
