...
- API는 요청마다 사용자 권한(`authenticated` 역할 + 검증한 토큰의 claims)으로 RLS를 켠 트랜잭션 안에서만 쿼리한다(`withUserDb`). 서비스 키로 쿼리하지 않는다
  - 예외는 둘뿐이고, 각각 전용 역할로 `private` 함수 하나만 부른다. 이 경로에 다른 쿼리를 추가하지 않는다
    - 확장 토큰 조회: `baro_token_resolver` → `private.resolve_api_token(해시)` (user_id만 돌려받음)
    - 속도 제한: `private.hit_rate_limit(...)` (버킷별 현재 횟수만 돌려받음). 이 함수의 EXECUTE는 `baro_rate_limiter`에만 주고 API 연결 계정은 그 역할의 멤버로서 상속받아 **한 문장으로** 부른다(`SET ROLE`을 하면 트랜잭션이 필요해 왕복이 4회가 된다). 요청이 실패해 롤백돼도 세야 하므로 라우트 트랜잭션 밖에서 돈다
- 확장 토큰(`baro_`)은 받겠다고 표시한 엔드포인트에서만 통한다(기본은 앱 토큰만, 지금은 `GET /me`·`POST /sync/chrome`). 토큰 발급·목록·폐기는 앱 토큰으로만
- `/sync/chrome` full의 대량 삭제는 확인(`confirmDeleteCount`) 없이 실행하지 않는다. 삭제는 방문 기록까지 지워 되돌릴 수 없다
