...
마지막 갱신: 2026-09-27 (SEARCH-05 완료·푸시·CI 통과. SET-01 열기 방식은 v1.1 후보로 옮김)
...
- [x] SEARCH-05: 정렬 설정 저장
  - [x] 설계 확정 (2026-09-27): `PATCH /me`는 `sortOption`만·4종·앱 토큰만, 바꿀 때마다 저장하되 한 번에 하나·마지막 값이 이김, 켤 때 목록과 설정이 둘 다 온 뒤 그리기, 저장 실패는 정렬 유지 + 문구
  - [x] 문서 먼저: 03-api.md(`PATCH /me` 절), 01-spec.md(SEARCH-05 줄, '정렬 규칙'의 저장·복원)
  - [x] 서버: `PATCH /me`(앱 토큰만) + `lib/me.ts`(GET·PATCH 공용) + shared `updateMeInput`·`SELECTABLE_SORTS`(4종. 기존 `SORT_OPTIONS` 5종은 'DB에 저장될 수 있는 값'으로 남김) + 테스트(shared 61, DB 3: 기본값·변경·다른 사용자 영향 없음)
  - [x] 앱: IPC `settings:getSort`·`settings:saveSort`(메인이 스키마로 재검사, 틀리면 요청 0건), `useSortSetting`(켤 때 한 번, 포커스로 다시 받지 않음), `useSortSaver`(한 번에 하나·마지막 값), 목록과 설정이 둘 다 온 뒤 그리기, 실패 문구. desktop 230 통과
  - [x] 실제 앱 확인 (2026-09-27, 테스트 계정)
    - [x] 정렬을 바꾸면 `PATCH /me` 200, DB 값 변경. 앱 안에서 20ms 사이 6번 바꾸면 `PATCH`는 2번(처음·마지막), DB는 마지막 값
    - [x] 복원: 새로고침 직후부터 섹션 이름을 기록 → 처음 그려진 그리드가 바로 저장된 정렬(330ms), 최근 추가순을 거치지 않음. 앱을 완전히 껐다 켜도 저장된 정렬로 시작(사람 확인)
    - [x] 저장 실패: API를 끄고 바꾸면 정렬은 바뀌고 상태바 '정렬을 저장하지 못했습니다', DB 그대로 → API를 켜고 다시 바꾸면 저장되고 문구 사라짐
    - [x] 정리: `sort_option`을 `created_desc`로(사람이 드롭다운으로)
    - 참고: 메모리 부족 뒤 남은 예전 앱 프로세스(electron-vite 9개)는 메인 변경을 반영하지 못해 모두 끄고 새로 띄움
- SET-01 열기 방식: **v1에서 뺌 → v1.1 후보로 옮김** (2026-09-27). `shell.openExternal`은 주소를 OS에 넘길 뿐이라 새 탭·현재 탭은 브라우저가 정한다. 앱이 제어할 수 없는 동작에 설정을 두지 않는다. 문서: 01-spec.md SET-01 줄, 03-api.md `PATCH /me`, 05-roadmap.md
...
- [ ] SET-02 테마
- [ ] SET-01 열기 방식(후보): 크롬 확장이 현재 탭을 바꾸는 방식(`tabs` 권한, 앱→확장 통로)으로 검토. 확장 권한 규칙(CLAUDE.md) 변경이 필요
- [ ] AUTH-04 회원 탈퇴
- [ ] 오래된 로그 정리 작업
