...
마지막 갱신: 2026-09-25 (6주차 SCR-03 설계 확정, 문서 먼저 반영)

**다음 작업 (순서대로)**
1. **6주차 디자인 토큰 정리 → SCR-03 + OPEN-01** (지금 홈은 여전히 임시 화면이다)
...
- [ ] 디자인 토큰 정리: `styles/tokens.css`로 모으기, 기존 화면(SCR-01·02·409 모달)도 토큰으로, `.preview-title` 중복 버그
  - [ ] 옮기기 전후 스크린샷 비교(SCR-01·SCR-02·409 모달)
  - [ ] `--btn-height`를 바꿔 모든 버튼이 바뀌는지 확인 후 원복
- [ ] SCR-03: 반응형 그리드, 파비콘 타일·글자 타일, 고정 섹션, 상단바(동기화·계정 메뉴), 상태바, 임시 홈·DevApiCard 삭제
  - [ ] `bookmarks:list` IPC — 화면 전에 콘솔에서 먼저 확인
  - [ ] TanStack Query 목록 캐시(동기화 후·창 포커스 갱신, 로그아웃 시 비우기)
  - [ ] 실제 앱 확인(창 너비별 열, 파비콘·글자 타일, 오프라인 재시작, 말줄임, 고정 섹션, 포커스 갱신, 409 모달, 로그아웃 후 캐시, 키보드, 빌드 버전 CSP)
- [ ] OPEN-01: 클릭 → 기본 브라우저로 열기
