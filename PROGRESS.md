...
마지막 갱신: 2026-09-21 (AUTH-01 완료: 앱에서 Google 로그인, profiles 자동 생성 확인)

**다음 작업: AUTH-02 — safeStorage 토큰 저장, 자동 로그인, 토큰 직접 갱신**
...
- [ ] AUTH-02: safeStorage 토큰 저장, 자동 로그인, 토큰 직접 갱신
  - [ ] 로그인된 상태에서 재로그인이 실패하면 기존 로그인 상태를 화면에 유지 (지금은 세션은 남고 화면에서만 이메일이 사라짐)
- [ ] AUTH-03: 로그아웃
- [ ] SCR-01 로그인 화면
- [ ] 앱 재실행 후에도 로그인 유지 확인
