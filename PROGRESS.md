...
마지막 갱신: 2026-09-22 (4주차: apps/extension 구현·자동 테스트, 운영 수동 확인 남음)

**다음 작업 (순서대로)**
1. apps/extension (EXT-01~04): manifest(key 고정), 팝업(토큰·연결 상태·전체 동기화, 409 확인), 이벤트 리스너
...
- [ ] apps/extension: manifest(key 고정), 팝업, 이벤트 리스너 (EXT-01~04)
  - 결정: EXT-02 포함, 409 미리보기는 서버에 추가, 실제 계정 확인은 별도 Google 테스트 계정으로만, 개발 DB 없음(단일 DB 사실대로 기록)
  - [ ] 문서: 01-spec.md '확장 동작 규칙'(구조·토큰·상태·변환·409·실시간), 03-api.md(409 `preview`, CORS 설명 정정), CLAUDE.md '크롬 확장 보안'
  - [ ] 서버: 409 `details.preview`(지워질 북마크 최대 5개 제목·URL)
  - [ ] 서버 테스트: 23505 재시도((a) 한 번만 반영 (b) 재시도도 실패면 500), confirmDeleteCount 39/40/41 경계, 413 출처(앱/Vercel)
  - [ ] 확장: MV3(권한 bookmarks·storage, host 2개, key로 ID `odhbbplpjlmkpggjocgiliebnmappfib`), 서비스 워커에서만 fetch·북마크 읽기, 팝업은 메시지만
  - [ ] 확장 자동 테스트(크롬 샘플 파일과 같은 트리, 가짜 storage·fetch)
  - [ ] 수동 확인(운영 API, 테스트 계정): 토큰 저장·401·첫 동기화·재동기화·409 확인·409 취소·재409·2MB·팝업 닫았다 열기·실시간 반영
