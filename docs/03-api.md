...
검색어 필터도 클라이언트에서 하므로 한 번에 전체를 받는다. 고정된 북마크가 항상 앞에 온다.
`recentVisits`는 최근 30일 방문 수(`visit_logs`를 북마크마다 세는 하위 쿼리, 인덱스 `idx_visit_recent`). **목록·단건·추가·수정 응답 모두에 붙는다**(앱이 수정·고정 응답으로 캐시 항목을 통째로 바꾸므로, 빠지면 자주 방문순에서 그 항목이 0회가 된다).
