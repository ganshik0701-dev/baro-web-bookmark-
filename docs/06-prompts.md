...
- PKCE로 CSRF 방지, 별도 state 없음 (code는 앱의 code_verifier로만 교환된다).
- 루프백 서버는 첫 callback만 받고 바로 닫는다. 타임아웃(2분) 처리를 넣어줘.
...
- 중복(409) 응답이면 "이미 있는 주소입니다" 안내와 함께 기존 북마크 열기/수정 또는 취소를 고르게 한다 (중복 저장은 허용하지 않음)
